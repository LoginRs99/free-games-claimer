...
async function getState() {
  ...
  const sched = getSchedulerConfig();
  const msActive = active.has('microsoft') || active.has('microsoft-mobile');
  const sgScheduled = active.has('steamgifts') && sched.sgFrequencyMinutes > 0;
  ...
  const schedEnabled = mainEnabled || msScheduled || sgScheduled || awaScheduled;
  ...
  const computedSg = sgScheduled ? new Date(Date.now() + computeSteamGiftsWakeMs()) : null;
  ...
  const effectiveNext = [effectiveMain, effectiveMs, effectiveSg, effectiveAwa]
    .filter(Boolean)
