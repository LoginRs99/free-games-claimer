...
export const SITES = [
  ...
  },
  {
    ...
    features: ['captcha-marker'],
    configFields: [
      { key: 'presenceMode', env: 'AWA_PRESENCE_MODE', type: 'string', default: 'auto_cap',
        label: 'AWA presence mode',
        hint: 'Auto-track until cap (recommended): Stays on Control Center and automatically reloads every 5 minutes to verify Time on Site ARP. Stops immediately as soon as the daily 5/5 ARP cap is reached. Fixed duration: runs for the exact minutes set below. Disabled: skips Time on Site presence.',
        options: [
          { value: 'auto_cap', label: 'Auto-track until daily cap (5/5 ARP)' },
          { value: 'fixed',    label: 'Fixed duration' },
          { value: 'disabled', label: 'Disabled' },
        ],
        coerce: { kind: 'stringEnum', values: ['auto_cap', 'fixed', 'disabled'], fallback: 'auto_cap' } },
      { key: 'presenceMinutes', env: 'AWA_PRESENCE_MINUTES', type: 'number', default: 25,
        label: 'AWA presence safety limit / duration',
        unit: 'minutes',
        hint: 'In Auto-track mode: safety runtime limit (default 25m) so the script never hangs if AWA servers fail to register points. In Fixed mode: exact duration to maintain presence.',
        coerce: { kind: 'numberBounded', min: 0, fallback: 25 } },
      { key: 'streamerSelectionMode', env: 'AWA_STREAMER_SELECTION_MODE', type: 'string', default: 'auto_2x',
        label: 'Streamer selection mode',
        hint: 'Auto 2x speed (recommended): Extracts live streamers directly from AWA Control Center Hive & Nexus sections (earns 2x ARP). Excludes standard 1x partners. Falls back to manual list if none are live.',
        options: [
          { value: 'auto_2x',      label: 'Auto 2x speed (Hive & Nexus live streamers)' },
          { value: 'manual_only',  label: 'Manual list only' },
        ],
        coerce: { kind: 'stringEnum', values: ['auto_2x', 'manual_only'], fallback: 'auto_2x' } },
      { key: 'stopOnTwitchCap', env: 'AWA_STOP_ON_TWITCH_CAP', type: 'boolean', default: true,
        label: 'Stop when Twitch ARP is maxed',
        hint: 'Immediately finishes the run as soon as Alienware Arena indicates daily Twitch ARP maximum is reached (underCap is false).',
        coerce: { kind: 'boolDefaultTrue' } },
      { key: 'dailyTargetMinutes', env: 'AWA_DAILY_TARGET_MINUTES', type: 'number', default: 120,
        label: 'Twitch max safety limit',
        unit: 'minutes',
        hint: 'Maximum safety runtime limit for Twitch stream watching. Runs stop earlier if Twitch ARP is maxed.',
        coerce: { kind: 'numberBounded', min: 1, fallback: 120 } },
      { key: 'arpTarget', env: 'AWA_ARP_TARGET', type: 'number', default: 0,
        label: 'ARP target',
        unit: 'ARP',
        hint: 'When total detected ARP balance is at or above this value, scheduled Alienware Arena runs skip. 0 disables.',
        coerce: { kind: 'numberBounded', min: 0, fallback: 0 } },
      { key: 'watchChunkMinutes', env: 'AWA_WATCH_CHUNK_MINUTES', type: 'number', default: 30,
        label: 'Twitch watch chunk',
        unit: 'minutes',
        hint: 'Watches one live streamer for this many minutes before re-checking AWA Control Center points and live list.',
      ...
      { key: 'twitchRecheckMinutes', env: 'AWA_TWITCH_RECHECK_MINUTES', type: 'number', default: 10,
        ...
        unit: 'minutes',
        hint: 'When nobody in the 2x list is live, Alienware Arena waits this many minutes before checking AWA again.',
        coerce: { kind: 'numberBounded', min: 1, fallback: 10 } },
      { key: 'twitchStreamers', env: 'AWA_TWITCH_STREAMERS', type: 'string',
        default: '',
        label: 'Fallback / manual Twitch streamers',
        hint: 'Optional comma-separated Twitch usernames used as fallback or when manual mode is selected. Default is empty (pure automatic tracking from Hive & Nexus).' },
